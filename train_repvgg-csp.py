...
def train(hyp, opt, device, tb_writer=None):
    ...
    names = ['item'] if opt.single_cls and len(data_dict['names']) != 1 else data_dict[
        'names']  # class names
    ...
    for epoch in range(start_epoch,
        ...
        if rank in [-1, 0]:
            ...
            if (not opt.nosave) or (final_epoch and not opt.evolve):  # if save
                ...
                if wandb_logger.wandb:
                    if ((
                                epoch + 1) % opt.save_period == 0 and not final_epoch) and opt.save_period != -1:
    ...
    if rank in [-1, 0]:
        ...
        if plots:
            ...
            if wandb_logger.wandb:
                ...
                wandb_logger.log({"Results": [wandb_logger.wandb.Image(str(save_dir / f), caption=f)
                                              for f in files
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='cfg/ablation/repvgg-csp.yaml',
                        help='model.yaml path')
    parser.add_argument('--data', type=str,
                        default='data/br35h.yaml',
                        help='data.yaml path')
    parser.add_argument('--hyp', type=str, default='data/hyp_training.yaml',
    ...
    parser.add_argument('--cache-images', action='store_false',
    ...
    parser.add_argument('--name', default='exp-repvgg-csp', help='save to project/name')
    ...
    if 'repvgg-csp' in opt.cfg:
        opt.hyp = 'data/hyp_training.yaml'
    ...
    if opt.resume and not wandb_run:  # resume an interrupted run
        ckpt = opt.resume if isinstance(opt.resume,
        ...
        opt.img_size.extend(
            [opt.img_size[-1]] * (2 - len(opt.img_size)))  # extend to 2 sizes (train, test)
        ...
        for _ in range(300):  # generations to evolve
            if Path('evolve.txt').exists():  # if evolve.txt exists: select best hyps and mutate
                ...
                while all(v == 1):  # mutate until a change occurs (prevent duplicates)
                    v = (g * (npr.random(ng) < mp) * npr.randn(ng) * npr.random() * s + 1).clip(0.3,
                                                                                                3.0)
