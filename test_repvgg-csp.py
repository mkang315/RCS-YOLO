...
def test(data,
    ...
    if training:  # called by train.py
        ...
        # Directories
        save_dir = Path(
            increment_path(Path(opt.project) / opt.name, exist_ok=opt.exist_ok))  # increment run
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            lb = [targets[targets[:, 0] == i, 1:] for i in
                  range(nb)] if save_hybrid else []  # for autolabelling
            t = time_synchronized()
            out = non_max_suppression(out, conf_thres=conf_thres * 200, iou_thres=iou_thres,
        ...
        for si, pred in enumerate(out):
            ...
            if len(
                    wandb_images) < log_imgs and wandb_logger.current_epoch > 0:  # Check for test operation
                if wandb_logger.current_epoch % wandb_logger.bbox_interval == 0:
                    box_data = [{"position": {"minX": xyxy[0], "minY": xyxy[1], "maxX": xyxy[2],
                                              "maxY": xyxy[3]},
                                 "class_id": int(cls),
        ...
        if plots and batch_i < 3:
            ...
            Thread(target=plot_images, args=(img, output_to_target(out), paths, f, names),
                   daemon=True).start()

    # Compute statistics
    stats = [np.concatenate(x, 0) for x in zip(*stats)]  # to numpy
    if len(stats) and stats[0].any():
        p, r, ap, f1, ap_class = ap_per_class(*stats, plot=plots, v5_metric=v5_metric,
    ...
    t = tuple(x / seen * 1E3 for x in (t0, t1, t0 + t1)) + (
        1000 / ((t0 + t1) / seen * 1E3), imgsz, imgsz, batch_size)  # tuple
    if not training:
        print(
            'Speed: %.1f/%.1f/%.1fms/%.1fFPS inference/NMS/total per %gx%g image at batch-size %g' % t)
    ...
    if save_json and len(jdict):
        w = Path(weights[0] if isinstance(weights,
                                          list) else weights).stem if weights is not None else ''  # weights
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=[
        'runs/train/exp-repvgg-csp/weights/best.pt', ],
                        help='model.pt path(s)')
    parser.add_argument('--data', type=str,
                        default='data/br35h.yaml',
                        help='*.data path')
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        ...
        for w in opt.weights:
            test(opt.data, w, opt.batch_size, opt.img_size, 0.25, 0.45, save_json=False,
